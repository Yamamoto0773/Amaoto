...
BOOL DirectXFigure::quad( size_t _vertexCnt, ... ) {
	int vertexCnt = (int)_vertexCnt;
	if (!lpDev)
		return FALSE;
	if (!pVertex)
		return FALSE;
...
BOOL DirectXFigure::GetPolyLineVertex( Point *pPolyLineVerList, Point *pPolyVerList, size_t polyVerCnt) {
	...
	for (i=1; i<polyVerCnt+1; i++) {
		polyLineVec[i-1].x = float(pPolyVerList[i%polyVerCnt].x - pPolyVerList[i-1].x);
...
FLOAT DirectXFigure::GetVec2Length( D3DXVECTOR2 *v ) {
