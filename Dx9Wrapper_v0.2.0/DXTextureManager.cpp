...
namespace dx9 {
	...
	DXTextureManager::DXTextureManager() :
		...
		textureAdjust(TextureAdjust::ASPECT_FIXED_REDUCEONLY) {}
	...
	bool DXTextureManager::CreateFromFile(Texture &tex, const std::wstring& fileName) {
		return CreateFromFile_main(tex, fileName, { 0, 0, 0, 0 }, false);
	}


	bool DXTextureManager::CreateFromFile(Texture &tex, const std::wstring& fileName, ClipArea clipArea) {
		return CreateFromFile_main(tex, fileName, clipArea, true);
	}
	...
	bool DXTextureManager::DrawTexture(const Texture & tex, float x, float y, float scale_x, float scale_y, float alpha, int rotDeg, bool isClip) {
		...
		case DrawTexCoord::TOP_R:
			pos.x = x - size.w*scale_x;
			pos.y = y;
			break;
		case DrawTexCoord::BOTTOM_R:
			pos.x = x - size.w*scale_x;
			pos.y = y - size.h*scale_y;
			break;
		...
		case DrawTexCoord::CENTER:
			pos.x = x - size.w*scale_x/2.0f;
			pos.y = y - size.h*scale_y/2.0f;
			break;
		default:
			break;
		}
	...
	bool DXTextureManager::DrawTexture(const Texture &tex, RectF posArea, ClipArea clipArea, float alpha, int rotDeg, bool isClip) {
		...
		UVCoord uv = tex.clipInfo.uv;
		uv.left += (float)clipArea.upleft_x/tex.tex->GetWidth();
		uv.top += (float)clipArea.upleft_y/tex.tex->GetHeight();
		uv.w = (float)clipArea.size_w/tex.tex->GetWidth();
		uv.h = (float)clipArea.size_h/tex.tex->GetHeight();
	...
	bool DXTextureManager::DrawTexture_main(const Texture & tex, float x, float y, float uv_left, float uv_top, float uv_w, float uv_h, float scale_x, float scale_y, float alpha, int rotDeg, bool isClip) {
		...
		Size size;
		size.w = (size_t)(tex.tex->GetWidth()*uv_w*scale_x);
		size.h = (size_t)(tex.tex->GetHeight()*uv_h*scale_y);
	...
	bool DXTextureManager::CreateFromFile_main(Texture & tex, const std::wstring & fileName, ClipArea clipArea, bool isClip) {
